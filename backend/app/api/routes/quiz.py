...
from app.schemas.public import QuestionPublic, QuestionsPublic

router = APIRouter('/quiz', tags=['quiz'])
...
def get_quiz_for_user(session: SessionDep, current_user: CurrentUser,):
    ...
    for q in internal_questions:
        ...
        # Create the public-facing response
        choices_with_ids = [
            {"id": chr(65 + i), "text": choice} for i, choice in enumerate(all_choices)
        ]
