...
const getScoreBgColor = (percentage: number) => {
  if (percentage === 0) return 'bg-[#4A5568] text-white'
      ...
      <div className='space-y-4'>
        {result.data.map((attempt, idx) => (
          <Link href={`/dashboard/courses/${courseId}/quiz-session/${attempt.id}`} className="p-4">
            <div
              key={attempt.id}
              className='flex items-center justify-between rounded-2xl border border-[#4A5568] bg-[#2D3748] p-4 transition-colors hover:bg-[#4A5568] h-[74px]'
            >
              <div className='flex items-center gap-6'>
                <div className='font-medium text-[#A0AEC0]'>#{idx + 1}</div>
